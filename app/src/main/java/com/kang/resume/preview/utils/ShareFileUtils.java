...
public class ShareFileUtils {
    ...
    private static void shareImage(Context context, String path, List<String> pathList, String pkg, String cls) {
        ...
        try {
            if (path != null) {
                //单张图片
                if (!RxFileTool.isFile(path)) {
                    RxToast.normal("图片不存在，请检查后重试");
                    return;
                }
                ...
                for (int i = 0; i < pathList.size(); i++) {
                    if (!RxFileTool.isFile(pathList.get(i))) {
                        RxToast.normal("第" + (i + 1) + "张图片不存在，请检查后重试");
                        return;
                    }
